...
import { apiMonitor } from '../middleware/apiMonitor';
...
router.get('/requests', (req, res) => {
  try {
    ...
    let requests = apiMonitor.getAllRequests();
    ...
    if (path) {
      requests = requests.filter(req => req.url.includes(path as string));
    }
...
router.get('/status', (req, res) => {
  try {
    const recentRequests = apiMonitor.getAllRequests().slice(0, 10);
    const stats = apiMonitor.getStats();
...
router.get('/requests/:id', (req, res) => {
  try {
    const { id } = req.params;
    const request = apiMonitor.getAllRequests().find(req => req.id === id);
...
router.get('/paths', (req, res) => {
  try {
    const requests = apiMonitor.getAllRequests();
